...
* **`GET /api/auth/migrate`**
  Запуск миграции БД для Сервиса аутентификации
...
* **`GET /api/tasks/migrate`**
  Запуск миграции БД для Сервиса работы с задачами
  **Доступ**: Role=admin
  **Параметры**: отсутствуют.
  **Ответ**: см. Примечание.
...
* **`GET /api/comments/{id}`**
  ...
  **Параметры**:
    * `id` — уникальный идентификатор задачи (целое число).
  **Ответ**: объект комментария или 404, если комментарий не найден.
...
* **`POST /api/tasks/{{task_id}}/attachments?fileName=example.txt`**
...
* **`GET /api/files?name=example.txt&path=file_path`**
  Получение файла (не объекта) вложения из хранилища.
  **Доступ**: аутентификация
  **Параметры**:
...
* **`POST /api/files`**
  Сохранение файла (не объекта) в хранилище.
  **Доступ**: аутентификация
  **Параметры**:
    * `file` — объект класса `AttachmentModel`, содержащий текстовые поля FilePath и FileName.
    * `file` - объект класса IFormFile.
  **Ответ**: объект, содержащий сообщение о результате сохранения и путь к файлу, или 500 Internal Server Error, если произошла ошибка.
